...
BOOST_AUTO_TEST_CASE(pass_dynamic_arguments_to_the_base_base)
{
	char const* sourceCode = R"(
		...
		contract Base1 is Base {
			constructor(uint k) Base(k*k) public {}
		}
		contract Derived is Base, Base1 {
			constructor(uint i) Base(i) Base1(i) public
			{}
		}
		contract Final is Derived(4) {
		}
...
BOOST_AUTO_TEST_CASE(function_array_cross_calls)
{
	char const* sourceCode = R"(
		contract D {
			function f(function() external returns (function() external returns (uint))[] x) public
